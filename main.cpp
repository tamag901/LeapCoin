...
#if defined(NDEBUG)
# error "Leapcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Leapcoin Signed Message:\n";
...
{
    // Leapcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Leapcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // 5 mins
static const int64 nTargetSpacing = 1 * 30; // 30 secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Leapcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Leapcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Leapcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FF Portal 14/Jun/2017 Square Enix joins the Red Cross to launch FINAL FANTASY XIV: Stormblood blood drive tour";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("897454710fa689ad5023690f56f3a49c8f13f8d45b8c857fbcbc8bc4a8e9d3eb4b10f4d4604fa68dce601aaf0f470216fe1b51850b4acf21b179c45070ac7beea9") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x8a7c77644005bb7678383b8d81fb55ea540a6ab26fd291f891739078f2de4e4e"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Leapcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LeapcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LeapcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LeapcoinMiner(CWallet *pwallet)
{
    printf("LeapcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("leapcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LeapcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LeapcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LeapcoinMiner, pwallet));
}
